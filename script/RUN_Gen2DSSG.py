...
exe_path='/home/sc/research/ORB_SLAM3/bin/orbslam3'
...
parser.add_argument('--save_name', type=str, default='graph_2dssg_orbslam3.json', help='', required=False)
parser.add_argument('--dataset',type=str,choices=['3RScan'],default='3RScan', help='type of dataset', required=True)
...
def process(pth_in, pth_out):
    startTime = time.time()
    try:
        output = subprocess.check_output([exe_path, args.config,
                                          '--pth_in',pth_in, 
...
if __name__ == '__main__':
    ...
    for scan_id in sorted(scan_ids):
        ...
        if not args.overwrite:
            if args.dataset == '3RScan':
                if os.path.isfile(os.path.join(define.DATA_PATH,scan_id,args.save_name)):
                    # print('skip',scan_id)
                    continue
            elif args.dataset == 'ScanNet':
                if os.path.isfile(os.path.join(define.SCANNET_DATA_PATH,scan_id,args.save_name)):
                    # print('skip',scan_id)
                    continue

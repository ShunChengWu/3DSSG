...
def process(pth_in, pth_out):
    ...
    try:
        output = subprocess.check_output([exe_path, args.config,
                                          '--pth_in',pth_in, 
                                          '--save_name_seq', args.save_name_seq,
                                          '--pth_out',pth_out,
                                          '--save_every','50',
                                          '--save_full','0'
                     ],
...
if __name__ == '__main__':
    ...
    for scan_id in sorted(scan_ids):
        ...
        if not args.overwrite:
            if args.dataset == '3RScan':
                if os.path.isfile(os.path.join(define.DATA_PATH,scan_id,args.save_name_seq)):
                    # print('skip',scan_id)
                    continue
            elif args.dataset == 'ScanNet':
                if os.path.isfile(os.path.join(define.SCANNET_DATA_PATH,scan_id,args.save_name_seq)):
                    # print('skip',scan_id)
                    continue
        ...
        print('pth_in:',pth_in)
        print('pth_out:',pth_out+'/'+args.save_name_seq)

...
def Parser(add_help=True):
    ...
    parser.add_argument('--scan_name', type=str, default='2dssg_seq', 
...
def process_one(args,
    ...
    # convert format
    ## Nodes
    for oid, obj_info in relationships['objects'].items():
        ascii_nn = [str(n).encode("ascii", "ignore") for n in segs_neighbors[oid]]
        output['nodes'][oid] = dict()
        output['nodes'][oid] = obj_info
        output['nodes'][oid]['neighbors'] = ascii_nn
    ## Relationships
    str_relationships = list() 
    for rel in relationships['relationships']:
        str_relationships.append([str(s) for s in rel])
    output['relationships'] = str_relationships
...
if __name__ == '__main__':
    ...
    with open(os.path.join(define.FILE_PATH + args.relation + ".json"), "r") as read_file:
        ...
        for s in tqdm(filtered_data):
            ...
            output = process(
                args,
                os.path.join('data','3RScan','data','3RScan'), 
                scan_id, 
                target_relationships,
                gt_relationships = gt_relationships)
            
            relationships_new[scan_id] = output
    ...
    for scan_id, scan_data in relationships_new.items():
        s_scan = str(scan_data)

...
pth_out = os.path.join(args.outdir,define.NAME_ROI_IMAGE)# '/media/sc/SSD1TB/dataset/3RScan/roi_images/'
pth_link = os.path.join(args.outdir,define.NAME_ROI_IMAGE+'.h5')

toTensor = transforms.ToTensor()
resize = transforms.Resize([256,256])
fdata =  cfg.data.path_3rscan_data#'./data/3RScan/data/3RScan'#define.DATA_PATH# '/media/sc/SSD1TB/dataset/3RScan/data/3RScan'
pattern = os.path.join(define.NAME_ROI_IMAGE,'*.h5')
    ...
    with h5py.File(path,'w') as h5f:
        for oid in node_ids:
            ...
            for fid in kf_indices:
                pth_rgb = os.path.join(fdata,scan_id,define.IMG_FOLDER_NAME, rgb_filepattern.format(fid))
                pth_pos = os.path.join(fdata,scan_id,define.IMG_FOLDER_NAME, pose_filepattern.format(fid))
                '''load data'''

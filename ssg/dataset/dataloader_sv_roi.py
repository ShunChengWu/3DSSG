...
class SingleViewROIImageLoader(data.Dataset):
    ...
    def __getitem__(self,idx):
        ...
        for idx in range(len(cat)):
            ...
            kf_indices = random_drop(range(img.shape[0]), 1, replace=True)
            img = img[kf_indices]
            img = torch.as_tensor(img)
            img = torch.clamp((img*255).byte(),0,255).byte()
            img = self.transform(img)
            img= normalize_imagenet(img.float()/255.0)
            # show_tensor_images(denormalize_imagenet(img), cls_label)
            img_list.append(img)

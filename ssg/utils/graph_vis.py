...
class DrawSceneGraph(object):
    ...
    def draw(self, pds:dict,gts:dict=None,node_colors=None):
        ...
        inst_groups = util_merge_same_part.collect(processed_pds['nodes'], processed_pds['edges'],define.NAME_SAME_PART)
            ...
            for k, v in gts['edges'].items():
                ...
                if name in inst_edge_gt:
                    if inst_edge_gt[name] != gts['edges'][k]:
                        inst_edge_gt[name]=inst_edge_gt[name].union(set(gts['edges'][k]))# = inst_edge_gt[name]+','+
        ...
        processed_pds['edges'] = process_edge_pd(inst_edge_pred) # replace to grouped edges
        ...
        if not grouping:
            ...
            for k in sorted_keys:
                ...
                if DRAW_MODE == 'SEMANTIC':
                    color = '#%02x%02x%02x' % label_color[util_label.nyu40_name_to_id(name_pd)+1]
            ...
            for edge in edge_pds.keys():
                ...
                if 'cluster' in f: f = f[7:]
                if f not in drawed_nodes: continue
                if 'cluster' in t: t = t[7:]
                if t not in drawed_nodes: continue
                ...
                for name_pd in names_pd:
                    g.edge(f,t,label=name_pd,color=self.color_missing_gt) # color_missing_pd
                i+=1
        else:
            for k,v in inst_groups.items():
                inst_name = 'cluster'+k
                with g.subgraph(name = inst_name) as c:
                    ...
                    for idx in v:
                        name_pd=none_name
                        if idx in node_pds:  name_pd = node_pds[idx]
    ...
    def draw_nogroup(self, pds:dict,node_colors):
        ...
        for edge in edge_pds.keys():
            ...
            if edge in edge_pds:
                names_pd = edge_pds[edge] if isinstance(edge_pds[edge],list) else [edge_pds[edge]]
            names_pd = set(names_pd).difference([define.NAME_NONE, 'same part'])
    ...
    def draw_evaluation(self, scan_id, pds,gts, none_name = define.NAME_NONE, 
        ...
        if inst_groups is None:
            ...
            for k,v in inst_groups.items():
                ...
                with g.subgraph(name = inst_name) as c:
                    ...
                    pre = []
                    for idx in v:
                        if seg_colors is not None and int(idx) not in seg_colors: continue
                        ...
                        if self.debug: node_label =str(idx)+'_'+node_label
        ...
        for edge in edges:
            ...
            if edge in edge_gts:
                names_gt = edge_gts[edge] if isinstance(edge_gts[edge],list) else [edge_gts[edge]]
            ...
            if len(names_gt) > 0:
                ...
                if pds != '' or gts != '':
                    if pds != '':pds+'\n'
                    g.edge(f,t,label=pds+'('+gts+')',color=self.color_wrong) # color_missing_pd

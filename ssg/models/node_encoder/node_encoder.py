...
class NodeEncoder(nn.Module):
    def __init__(self, cfg,device):
        ...
        #     self.fc = nn.Linear(cfg.model.img_feature_dim*size, self.node_feature_dim)
            ...
            self.conv1 = nn.Conv2d(cfg.model.img_feature_dim, hidden_size, kernel_size=self.roi_region, stride=1)
            self.conv2 = nn.Conv2d(hidden_size, self.node_feature_dim, kernel_size=1, stride=1)
            
            self.use_drop_out = drop_out>0
            if self.use_drop_out:
                self.dropout = nn.Dropout(p=drop_out)
            if self.with_bn:
                self.bn1 = nn.BatchNorm2d(hidden_size, eps=0.001, momentum=0.01)
                self.bn2 = nn.BatchNorm2d(self.node_feature_dim, eps=0.001, momentum=0.01)
...
class NodeEncoderVGG16(nn.Module):
    def __init__(self, cfg,device):
        ...
        if self.node_feature_dim != 4096:
            logger_py.warning("self.node_feature_dim must have size 4096. add a projection layer to project final feature")
            self.proj = nn.Linear(4096, self.node_feature_dim)
        else:
            self.proj = nn.Sequential()
        
        if self.roi_region[0] != 7:
            logger_py.warning("roi region must be 7 when using NodeEncoderVGG16. change it from {} to 7".format(self.roi_region[0]))
    ...
    def forward(self, images, bboxes, edges, **args):
        ...
        if self.with_precompute:
            ...
            with torch.no_grad():
                ...
                img_features = torch.cat([ self.encoder(p_split)  for p_split in torch.split(images,int(4), dim=0) ], dim=0)
        ...
        for node_idx in range(n_nodes):
            ...
            roi_features = roi_align(selected_img_features, boxes, self.roi_region)
            roi_features = self.fc(roi_features.view(roi_features.shape[0],-1))
...
class NodeEncoderRes18(nn.Module):
    def __init__(self, cfg,device):
        ...
        if self.node_feature_dim != 512:
            logger_py.warning("self.node_feature_dim must have size 512. add a projection layer to project final feature")
            self.proj = nn.Linear(512, self.node_feature_dim)
        else:
            self.proj = nn.Sequential()
        
        if self.roi_region[0] != 1:
            logger_py.warning("roi region must be 1 when using NodeEncoderRes18. change it from {} to 1".format(self.roi_region[0]))

...
def parse():
    ...
    parser.add_argument('-f','--filename', type=str, default='/home/sc/research/ORB_SLAM3/bin/test/graph_2dssg_seq.json', 
                        help='filepath to graph_2dssg_seq.json',required=True)
    parser.add_argument('-v','--vis', action='store_true',help='show scene graph')
...
def main():
    ...
    if not with_loader:
        ...
        path = cfg.filename
        # path = '/media/sc/SSD1TB/dataset/3RScan/data/3RScan/4acaebcc-6c10-2a2a-858b-29c7e4fb410d/graph_2dssg_seq.json'
            ...
            with torch.no_grad():
                ...
                if pt>0:
                    ...
                    g = graphDrawer.draw({'nodes':nodes_pds_all, 'edges':edges_pds_all},
                                          gts,node_colors=node_colors_all)
                    g.render(os.path.join(cfg['training']['out_dir'], 
                                              cfg.name, str(fid)+'_graph'),view=cfg.vis)
        print('done')
    else:
...
class InputProcessor(object):
    ...
    def __call__(self,path_graph):
        ...
        debug = self.debug
        for scan_id, incre_data in sdata.items():
            keys = [int(k) for k in incre_data]
            keys = sorted(keys)
            
            for key in keys:
                if key < 0: continue
                data = incre_data[str(key)]
                nodes = data['nodes']
